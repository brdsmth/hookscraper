...
async function cycleLoadMore(page) {
  ...
  do {
    ...
    const loadMoreButton = await page.$(
      "#yDmH0d > c-wiz > div > div > div > div > main > div > div.acFDHd > div > div.mMhoGc > div > button",
    )
    if (loadMoreButton) {
      await loadMoreButton.click()
      await page.waitForTimeout(1000)
      totalElements = await page.$$eval(
        ".UvhDdd",
...
async function cycleClickEmail(page) {
  ...
  const dropdownButton = await page.$(
    "#yDmH0d > c-wiz > div > div > main > div > section:nth-child(5) > div:nth-child(2) > div > ul > li.Qt4bne.Lj9Zzc > div:nth-child(2) > div > details > summary",
  )
  if (dropdownButton) {
    await dropdownButton.click()
    // Wait for the email to appear after clicking the dropdown
    await page.waitForSelector(
      "#yDmH0d > c-wiz > div > div > main > div > section:nth-child(5) > div:nth-child(2) > div > ul > li.Qt4bne.Lj9Zzc > div:nth-child(2) > div > details > div",
      { timeout: 1000 },
  ...
  for (let i = 0; i < totalElements; i++) {
    ...
    try {
      ...
      const extensionName = await page
        .waitForSelector("h1.Pa2dE", { timeout: 1000 })
        .then(() => page.$eval("h1.Pa2dE", (el) => el.textContent))
        .catch(() => "")

      // Wait for extension URL
      const extensionUrlHref = await page
        .waitForSelector("a.cJI8ee", { timeout: 1000 })
        .then(() => page.$eval("a.cJI8ee", (el) => el.href))
        .catch(() => "")

      // Wait for extension rating
      const extensionRating = await page
        .waitForSelector("span.Vq0ZA", { timeout: 1000 })
        .then(() => page.$eval("span.Vq0ZA", (el) => el.textContent))
        .catch(() => "")

      // Wait for number of ratings
      const numRatings = await page
        .waitForSelector("p.xJEoWe", { timeout: 1000 })
        .then(() => page.$eval("p.xJEoWe", (el) => el.textContent))
        .catch(() => "")

      const lastUpdated = await page
        .waitForSelector("li.Qt4bne.kBFnc > div:nth-child(2)", {
          timeout: 1000,
        })
        .then(() =>
          page.$eval(
            "li.Qt4bne.kBFnc > div:nth-child(2)",
            (el) => el.textContent,
          ),
        )
        .catch(() => "")
      ...
      const divContent = await page
        .waitForSelector("div.F9iKBc", { timeout: 1000 })
